...
def getNodePoints(nodeid, childs, nodeR = 10):
    ...
    for t in range(-math.pi, -math.pi, inc):
...
def getMatrix():
    ...
    elements = fixPosition(elements)


    return elements
def scale(p, max = 0, min=0, srcmin=-1, srcmax=1):
    scaled = float(min) + float(max-min) * float(p - srcmin)/float(srcmax - srcmin)

    return scaled

def fixPosition(elements=[]):
    ...
    for element in elements:
        ...
        if 'nodes' == grp:
            elid = element['data']['id']
            if elid in positions:
                x, y = positions[elid]
                x = scale(x, max=width)
                y = scale(y, max=height )
                element['position'] = dict(x = x, y=y)
